# ADR-13 — Chicago-Style Unit Testing Strategy
...
- Test matchers with real collections of temporal events.
...
- Fewer dependencies on mocking frameworks (aligns with ADR-7: no external dependencies).
...
- No external dependencies exist that would require mocking (ADR-7).
